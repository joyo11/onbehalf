...
import { EXCLUDE_COMPANIES, TARGET_LOCATIONS, TARGET_ROLES } from "@/lib/data";

export default function SearchScreen() {
  const router = useRouter();
  const [keywords, setKeywords] = useState<string[]>([...TARGET_ROLES]);
  const [locations, setLocations] = useState<string[]>([...TARGET_LOCATIONS]);
  const [excluded, setExcluded] = useState<string[]>([...EXCLUDE_COMPANIES]);
  const [salary, setSalary] = useState<number>(180);
